  ...
  return (
    <div>
      ...
      <div id="carousel-track" ref={trackRef} style={{ display: "flex", overflowX: "auto", scrollSnapType: "x mandatory", scrollbarWidth: "none", gap: 16, paddingBottom: 4 }}>
        {SLIDES.map((s, i) => (
          <article key={i} style={{ flex: "0 0 clamp(260px, 32%, 360px)", scrollSnapAlign: "start", background: "#08090b", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "24px 22px", display: "flex", flexDirection: "column", gap: 12 }}>
            <span style={{ fontFamily: MONO, fontSize: "0.68rem", letterSpacing: "1px", textTransform: "uppercase" as const, color: "#3b82f6" }}>{s.tag}</span>
            <h4 style={{ fontFamily: HEAD, fontWeight: 700, fontSize: "1.1rem", color: "#f3f4f6", margin: 0 }}>{s.title}</h4>
            <p style={{ color: "#8b919c", fontSize: "0.88rem", lineHeight: 1.65, flex: 1, margin: 0 }}>{s.body}</p>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 14 }}>
              <span style={{ fontFamily: MONO, fontSize: "0.68rem", letterSpacing: "1.5px", color: "#565c66" }}>{s.client}</span>
              <span style={{ fontFamily: MONO, fontSize: "0.74rem", color: "#38e8b0" }}>{s.outcome}</span>
            </div>
          </article>
        ))}
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 18 }}>
        <div style={{ display: "flex", gap: 7 }}>
          {SLIDES.map((_, i) => (
            <button key={i} onClick={() => go(i)} style={{ width: active === i ? 22 : 7, height: 7, borderRadius: 4, border: "none", cursor: "pointer", background: active === i ? "#3b82f6" : "rgba(255,255,255,0.12)", transition: "all .25s", padding: 0 }} />
          ))}
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          {[{ l: "←", d: -1 }, { l: "→", d: 1 }].map(({ l, d }) => (
            <button key={l} onClick={() => go(active + d)} style={{ width: 40, height: 40, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.04)", color: "#f3f4f6", fontSize: "1rem", cursor: "pointer", display: "grid", placeItems: "center" }}>{l}</button>
          ))}
        </div>
      </div>
    </div>
...
function DispatchConsole() {
  ...
  return (
    <div style={{ background: "#08090b", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 14, overflow: "hidden", fontFamily: MONO }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.025)" }}>
        {["#ff6b6b","#ffb224","#38e8b0"].map(c => <span key={c} style={{ width: 11, height: 11, borderRadius: "50%", background: c, opacity: c === "#ff6b6b" ? 0.8 : 0.4 }} />)}
        <span style={{ marginLeft: 8, fontSize: "0.7rem", color: "#565c66", letterSpacing: "1px" }}>dispatch — live</span>
      </div>
      <div style={{ padding: 18 }}>
        {[{ label: "orders_in_queue", val: "14", color: "#ffb224" }, { label: "auto_routed_today", val: "1,986", color: "#38e8b0" }, { label: "exceptions", val: "2 pending", tag: true }].map(r => (
          <div key={r.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
            <span style={{ fontSize: "0.72rem", color: "#565c66" }}>{r.label}</span>
            {r.tag ? <span style={{ fontFamily: MONO, fontSize: "0.66rem", padding: "3px 8px", borderRadius: 4, background: "rgba(255,107,107,0.12)", color: "#ff6b6b", border: "1px solid rgba(255,107,107,0.25)" }}>{r.val}</span>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 5, height: 52, marginTop: 16, marginBottom: 12 }}>
          {BARS.map((h, i) => <div key={i} style={{ flex: 1, background: i === 5 ? "rgba(59,130,246,0.65)" : "rgba(59,130,246,0.18)", borderRadius: "3px 3px 0 0", height: `${h}%`, border: "1px solid rgba(59,130,246,0.22)", borderBottom: "none" }} />)}
        </div>
        <div style={{ fontSize: "0.7rem", color: "#565c66", paddingTop: 10, borderTop: "1px solid rgba(255,255,255,0.05)" }}>
          <span style={{ color: "#3b82f6" }}>engine ▸</span> status: balanced
          <span style={{ marginLeft: 4, display: "inline-block", width: 6, height: 12, background: "#3b82f6", verticalAlign: "middle", opacity: tick ? 0.9 : 0, transition: "opacity .15s" }} />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <ThemeHeroSection>
        ...
        <h1 className="hero-anim-3" style={{ fontFamily: HEAD, fontWeight: 700, fontSize: "clamp(2rem,5vw,3.8rem)", lineHeight: 1.06, letterSpacing: "-0.025em", color: "#f3f4f6", maxWidth: "15ch", margin: "0 0 20px" }}>
          Systems quietly <span style={{ color: "#3b82f6" }}>doing the work</span> right now.
        </h1>
      ...
      {/* ── Client Logos ── */}
      <section className="relative z-10 py-10 md:py-14" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="w-full max-w-[1200px] mx-auto px-5 sm:px-8 lg:px-10">
          <div style={{ marginBottom: 36 }}>
            <Eyebrow>Clients</Eyebrow>
            <h2 style={{ fontFamily: HEAD, fontWeight: 700, fontSize: "clamp(1.8rem,3vw,2.4rem)", letterSpacing: "-0.02em", color: "#f3f4f6", margin: 0 }}>Teams that trust the machinery.</h2>
          </div>
          <div ref={logosReveal.ref} className="grid gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-5" style={{ opacity: logosReveal.on ? 1 : 0, transition: "opacity .7s" }}>
            {CLIENTS.map((c, i) => (
              <div key={c} style={{ border: "1px solid rgba(255,255,255,0.07)", borderRadius: 9, padding: "16px 10px", textAlign: "center", fontFamily: MONO, fontSize: "0.68rem", letterSpacing: "2px", color: "#565c66", opacity: logosReveal.on ? 1 : 0, transition: `opacity .6s ${i * 0.05}s` }}>{c}</div>
            ))}
          </div>
        </div>
      </section>
          ...
          <div ref={featuredReveal.ref} style={{ opacity: featuredReveal.on ? 1 : 0, transition: "opacity .7s" }}>
            <div className="grid gap-10 md:gap-16 md:grid-cols-2 md:items-center">
              <div>
                ...
                <h3 style={{ fontFamily: HEAD, fontWeight: 700, fontSize: "clamp(1.7rem,3vw,2.4rem)", letterSpacing: "-0.02em", lineHeight: 1.15, color: "#f3f4f6", marginBottom: 18 }}>From 6 people routing orders to 0.</h3>
                ...
                <ul style={{ listStyle: "none", padding: 0, margin: "0 0 26px", display: "flex", flexDirection: "column", gap: 10 }}>
                  {["2,000+ orders auto-routed every day","Exceptions surfaced in a live queue","Full audit trail for every decision"].map(item => (
                    <li key={item} style={{ display: "flex", alignItems: "center", gap: 12, color: "#8b919c", fontSize: "0.9rem" }}>
                ...
                <Link href="/onboard" style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: BODY, fontWeight: 600, fontSize: "0.9rem", padding: "11px 20px", borderRadius: 100, color: "#f3f4f6", border: "1px solid rgba(255,255,255,0.16)", textDecoration: "none" }}>Start something like this</Link>
              </div>
              <DispatchConsole />
            </div>
          </div>
        ...
        <div className="w-full max-w-[1200px] mx-auto px-5 sm:px-8 lg:px-10">
          <div ref={ctaReveal.ref} style={{ position: "relative", textAlign: "center", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: "clamp(48px,8vw,80px) clamp(24px,4vw,48px)", overflow: "hidden", background: "#08090b", opacity: ctaReveal.on ? 1 : 0, transform: ctaReveal.on ? "translateY(0)" : "translateY(20px)", transition: "opacity .7s, transform .7s" }}>
            ...
            <div style={{ position: "relative", zIndex: 1 }}>
              <h2 style={{ fontFamily: HEAD, fontWeight: 700, fontSize: "clamp(1.6rem,3vw,2.4rem)", letterSpacing: "-0.02em", lineHeight: 1.2, color: "#f3f4f6", maxWidth: "22ch", margin: "0 auto 28px" }}>Your project could be the next one on this page.</h2>
              <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
                ...
                <Link href="/contact" style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: BODY, fontWeight: 600, fontSize: "0.9rem", padding: "12px 20px", borderRadius: 100, color: "#f3f4f6", border: "1px solid rgba(255,255,255,0.16)", textDecoration: "none" }}>Book a call</Link>
              </div>
            </div>
          </div>
        </div>
